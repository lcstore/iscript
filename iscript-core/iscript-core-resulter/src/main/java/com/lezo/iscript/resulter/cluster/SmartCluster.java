...
public class SmartCluster {
	...
	private List<CenterToken> assortCenters(List<CenterToken> centerList) {
		...
		for (CenterToken center : centerList) {
			EntityToken scatter = hasScattered(center);
			if (scatter != null) {
				newCenters.add(new CenterToken(scatter));
			}
		}
	...
	private EntityToken hasScattered(CenterToken center) {
		...
		int lowCount = 0;
		EntitySimilar scatter = null;
		for (EntitySimilar sm : center.getSimilars()) {
			if (sm.getSimilar() < MAX_SCATTERED_SIMILAR) {
				if (scatter == null || scatter.getSimilar() > sm.getSimilar()) {
					scatter = sm;
				}
				lowCount++;
			}
		}
		if (lowCount * 100 / total >= 50) {
			return scatter.getRightCover().getEntity();
		}
		return null;
	}
