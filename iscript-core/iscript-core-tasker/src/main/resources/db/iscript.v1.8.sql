...
CREATE TABLE `T_BRAND` (
  ...
  `SYNONYM_CODE` varchar(20) NOT NULL DEFAULT '' COMMENT '同义词编码',
  `REGION` varchar(50) NOT NULL DEFAULT '' COMMENT '地区',
  `CREATE_TIME` timestamp NULL  COMMENT '创建时间',
  `UPDATE_TIME` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00' ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`ID`),
  KEY `IDX_T_BRAND_SITE_CODE` (`SITE_ID`,`BRAND_CODE`),
  KEY `IDX_T_BRAND_SITE_SYNCODE` (`SITE_ID`,`SYNONYM_CODE`)
...
CREATE TABLE `T_BRAND_SHOP` (
  ...
  `UPDATE_TIME` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00' ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`ID`),
  KEY `IDX_T_BRAND_SHOP_NAME_BCODE` (`SHOP_NAME`,`BRAND_CODE`)
